...
//
// version modified for paper plots Feb. 22, 2021
//
////////////////////////////////////////////////////////////////////
...
const int plotBunch = 1112; // bunch to actually save plots for
...
const std::string separationFileName = "TrackLumiData/VdMSteps_"+fillNumber+"_AllScans.txt";
...
std::tuple<float, float, float, float> PlotTrackLumiVdMPaper(const char *scanFileName, std::string outFileName, const char *plotTitle, int channelNum = -1, bool savePlot = false) {
  ...
  TLegend *l1 = new TLegend(0.6, 0.73, 0.9, 0.88);
  ...
  t2->SetTextSize(0.05);
  t2->Draw();
  TText *t3 = new TText(0, 0, "Preliminary");
  t3->SetNDC();
  t3->SetX(0.25);
  t3->SetY(0.84);
  t3->SetTextFont(52);
  ...
  if (savePlot) {
    c1->Print((outFileName+".png").c_str());
    c1->Print((outFileName+".pdf").c_str());
  }
...
void PlotTrackLumiVdMPaper(int whichPlot) {
  ...
  if (whichPlot == 0) {
    PlotTrackLumiVdMPaper(("TrackLumiData/TrackLumiZC_"+fillNumber+"_X"+scanPair+".txt").c_str(), ("TrackLumi_VdM_"+fillNumber+"_X"+scanPair+"_All"),
		   ("Fill "+fillNumber+", Scan X"+scanPair+", all bunches").c_str());
    PlotTrackLumiVdMPaper(("TrackLumiData/TrackLumiZC_"+fillNumber+"_Y"+scanPair+".txt").c_str(), ("TrackLumi_VdM_"+fillNumber+"_Y"+scanPair+"_All"),
    ...
    if (whichPlot == 1) {
      ...
      capSigmaFileName = "TrackLumi_VdM_"+fillNumber+"_"+scanPair+"_CapSigmas_BX";
      sigmaVisPlotTitle = "Visible cross section per bunch, fill "+fillNumber+", scan pair "+scanPair+", all channels";
      sigmaVisFileName = "TrackLumi_VdM_"+fillNumber+"_"+scanPair+"_SigmaVis_BX";
    } else if (whichPlot == 2) {
      nmax = nPixelChannels;
      ...
      capSigmaFileName = "TrackLumi_VdM_"+fillNumber+"_"+scanPair+"_CapSigmas_Chan";
      sigmaVisPlotTitle = "Visible cross section per channel, fill "+fillNumber+", scan pair "+scanPair+", all bunches";
      sigmaVisFileName = "TrackLumi_VdM_"+fillNumber+"_"+scanPair+"_SigmaVis_Chan";
	...
	if (beam1Intensity[i] > 1e8 && beam2Intensity[i] > 1e8) {
      ...
      capSigmaFileName = "TrackLumi_VdM_"+fillNumber+"_"+scanPair+"_CapSigmas_Chan_BX81";
      sigmaVisPlotTitle = "Visible cross section per channel, fill "+fillNumber+", scan pair "+scanPair+", BX 81";
      sigmaVisFileName = "TrackLumi_VdM_"+fillNumber+"_"+scanPair+"_SigmaVis_Chan_BX81";
	...
	fileStringX = "TrackLumiData/TrackLumi_"+fillNumber+"_X"+scanPair+"_BX/TrackLumiZC"+bxStringPad.str()+".txt";
	plotStringX = "TrackLumi_VdM_"+fillNumber+"_X"+scanPair+"_"+bxStringPad.str();
	titleStringX = "Fill "+fillNumber+", Scan X"+scanPair+", bunch "+bxString.str();
	
	fileStringY = "TrackLumiData/TrackLumi_"+fillNumber+"_Y"+scanPair+"_BX/TrackLumiZC"+bxStringPad.str()+".txt";
	plotStringY = "TrackLumi_VdM_"+fillNumber+"_Y"+scanPair+"_"+bxStringPad.str();
	...
	plotStringX = "TrackLumi_VdM_"+fillNumber+"_X"+scanPair+"_Ch"+chanString.str();
	...
	plotStringY = "TrackLumi_VdM_"+fillNumber+"_Y"+scanPair+"_Ch"+chanString.str();
	...
	plotStringX = "TrackLumi_VdM_"+fillNumber+"_X"+scanPair+"_Ch"+chanString.str()+"_BX81";
	...
	plotStringY = "TrackLumi_VdM_"+fillNumber+"_Y"+scanPair+"_Ch"+chanString.str()+"_BX81";
	...
	beamIntensityProduct = overallBunchProduct;
      ...
      bool savePlot = (bunches[i] == plotBunch);
       
      // Do the actual calls to read and fit the data.
      std::tuple<float, float, float, float> resultsX = PlotTrackLumiVdMPaper(fileStringX.c_str(), plotStringX, titleStringX.c_str(), targetChan, savePlot);
      std::tuple<float, float, float, float> resultsY = PlotTrackLumiVdMPaper(fileStringY.c_str(), plotStringY, titleStringY.c_str(), targetChan, savePlot);
    ...
    TLegend *l = new TLegend(0.65, 0.73, 0.9, 0.88);
    ...
    cs->Print((capSigmaFileName+".png").c_str());
    cs->Print((capSigmaFileName+".pdf").c_str());
    ...
    csv->Print((sigmaVisFileName+".pdf").c_str());
  }
}

// Default to "1" for paper
void PlotTrackLumiVdMPaper() {
  PlotTrackLumiVdMPaper(1);
}
